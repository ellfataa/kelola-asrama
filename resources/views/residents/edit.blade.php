            ...
            <div class="p-8">
                ...
                @php
                    ...
                    foreach($rooms as $r) {
                        $roomsData[$r->id] = [
                            'capacity' => $r->capacity,
                            // Array bed yang terisi (kecuali diri sendiri)
                ...
                <form action="{{ route('residents.update', $resident) }}" method="POST"
                    ...
                    {{-- ... (Bagian Form Nama, NIM, HP sama seperti create) ... --}}
                    {{-- Saya ringkas disini agar fokus ke logic bed --}}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                             ...
                             <input type="date" name="entry_date" value="{{ old('entry_date', $resident->entry_date->format('Y-m-d')) }}" class="block w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 py-3 transition-all duration-200" required>
                        </div>
                    </div>

                    {{-- BAGIAN PINDAH KAMAR / BED --}}
                    <div class="border-b border-slate-100 pb-4 pt-4 mb-4">
                        <h3 class="text-lg font-bold text-slate-800">Alokasi Kamar</h3>
                        <p class="text-sm text-slate-500">Pilih unit kamar dan slot tempat tidur baru.</p>
                    </div>

                    <div>
                        <label class="block font-bold text-xs text-slate-500 uppercase tracking-wider mb-2">Pilih Unit Kamar</label>
                        {{-- Saat ganti kamar, reset selectedBed ke null --}}
                        <select name="room_id" x-model="selectedRoom" @change="selectedBed = null"
                            ...
                            @foreach($rooms as $room)
                                @php
                                    ...
                                    // Sisa slot ditambah 1 jika ini kamar saya (karena saya dihitung filled)
                                    $sisa = $room->capacity - $filled + ($isMyRoom ? 1 : 0);
                                @endphp
                                <option value="{{ $room->id }}" {{ $sisa <= 0 ? 'disabled' : '' }} class="{{ $sisa <= 0 ? 'bg-slate-100 text-slate-400' : '' }}">
                                    Unit {{ $room->number }} {{ $isMyRoom ? '(Kamar Saat Ini)' : '(Sisa ' . $sisa . ' Slot)' }} - Rp {{ number_format($room->price, 0, ',', '.') }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div x-show="selectedRoom" x-transition class="mt-6 bg-slate-50 border border-slate-200 rounded-2xl p-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            <template x-for="i in getCapacity()">
                                <div @click="!isBedTaken(i) ? selectedBed = i : null"
                                     :class="{
                                         'bg-slate-200 opacity-50 cursor-not-allowed': isBedTaken(i),
                                    ...
                                    <svg class="w-6 h-6 mb-1" :class="isBedTaken(i) ? 'text-slate-400' : (selectedBed == i ? 'text-indigo-600' : 'text-slate-300')" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 14a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H9a2 2 0 01-2-2v-4z M7 14V6a2 2 0 012-2h8a2 2 0 012 2v8 M7 14h10 M7 10h10 M7 6h10"></path></svg>
                                    <span class="text-xs font-bold" x-text="'Slot ' + i"></span>
                                    
                                    {{-- Status Label --}}
                                    <span x-show="isBedTaken(i)" class="absolute top-1 right-1 text-[8px] bg-slate-300 px-1 rounded text-slate-600 font-bold">ISI</span>
                                    <span x-show="selectedBed == i" class="absolute top-1 right-1">
                        ...
                        <div x-show="!selectedBed" class="mt-3 text-xs text-red-500 text-center font-medium animate-pulse">* Wajib memilih salah satu slot bed</div>
